...
const Slider: React.FC = () => {
    ...
    const onClickNext = ()=> {
        count>=photo.length-1 ? setCount(photo.length-1) : setCount(count+=1);
    }
        ...
        <div ref={slider} className={style.container}>
            ...
            <div style={{width: width}} className={style.slider}>
                <ul ref={sliderLine} className={style.slider__line} style={{
                    ...
                    {photo.map((item, index) => <li className={style.slider__line_item} key={index}><img style={{width: width}} src={item.url} alt="" /></li>)}
                </ul>
            </div>
